...
open class SteppedProgressBar: UIView {
    ...
    open var titles = ["البيانات والدفع", "الشروط والاحكام","اختيار الحجز", "حجز الفترة","حجز التاريخ"] {
        didSet {
            self.setNeedsDisplay()
        }
    }
    ...
    func draw(step i: Int, path: UIBezierPath, start point: inout CGPoint, textColor: UIColor) {
        ...
        if stepDrawingMode == .drawIndex {
            ...
            let font = UIFont.boldSystemFont(ofSize: 4.0)
        ...
        attributes[NSFontAttributeName] = UIFont.boldSystemFont(ofSize: 7.0)
